...
basist::etc1_global_selector_codebook* g_basisut_cookbook;
basist::basisu_transcoder* g_basisut_transcoder;

void basisut_init()
{
    g_basisut_cookbook =
        new (sx_malloc(the__core.heap_alloc(), sizeof(basist::etc1_global_selector_codebook)))
    ...
    g_basisut_transcoder =
        new (sx_malloc(the__core.heap_alloc(), sizeof(basist::basisu_transcoder)))
...
void basisut_release()
{
    ...
    sx_free(the__core.heap_alloc(), g_basisut_transcoder);
    g_basisut_cookbook->~etc1_global_selector_codebook();
    sx_free(the__core.heap_alloc(), g_basisut_cookbook);
}
...
{
    return g_basisut_transcoder->start_transcoding(data, data_size);
}
...
{
    return g_basisut_transcoder->transcode_image_level(
        data, data_size, image_index, level_index, pOutput_blocks,
        output_blocks_buf_size_in_blocks_or_pixels, (basist::transcoder_texture_format)fmt,
        decode_flags);
}
