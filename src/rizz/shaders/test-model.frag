...
layout (location=TEXCOORD0) in vec3 v_normal;

layout (location = SV_Target0) out vec4 frag_color;

layout (binding=0, std140) uniform mesh_uniforms_fs {
    vec4 light_dir_;
};

#define light_dir light_dir_.xyz

vec3 eval_sh(vec3 _dir)
{
...
void main() {
    // float lit = max(0, dot(normalize(v_normal), light_dir));
    vec3 lit = eval_sh(normalize(v_normal));
